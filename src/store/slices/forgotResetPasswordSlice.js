...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      "https://mernstact-portfolio-backend-code-1.onrender.com/api/v1/user/password/forgot",
      { email },
  ...
  (token, password, confirmPassword) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        ` https://mernstact-portfolio-backend-code-1.onrender.com/api/v1/user/password/reset/${token}`,
